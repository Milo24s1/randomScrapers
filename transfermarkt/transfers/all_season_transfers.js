 ...
 * @description
 * HOW TO RUN SCRIPT
 * node all_season_squad.js <URL> <LIMIT>
 * example ( node all_season_transfers.js https://www.transfermarkt.co.uk/fc-barcelona/transfers/verein/131 20
 *
 * path issue fixed
 *
 * @author
...
function main(getOption) {
    try {
        ...
        request.get(getOption,async(err,response,html)=>{
            if(err){
                console.log('Error in request sending');
                console.log(err);
            }
            else {
                console.log('initial response processing');
                const seasonList = getSeasonList(html);
                const formattedInputUrl = getFomattedInputUrl(getOption.url);
                const teamId = formattedInputUrl.split("/").pop();

                if(LIMIT===undefined){
                    LIMIT = seasonList.length;
                }

                let currentIndex =1;
                for (let season of seasonList){
                    const currentSeasonYear = season['year'];
                    /**
                     * urlBySession = 'https://www.transfermarkt.co.uk/fc-barcelona/transfers/verein/131/saison_id/2019/plus/1';
                     * */
                    getOption.url = formattedInputUrl+'/saison_id/'+currentSeasonYear+'/plus/1';
                    processSeason(getOption,teamId,currentSeasonYear);
...
function processSeason(getOption,teamId,currentSeasonYear) {
    try {

        console.log('processing season '+currentSeasonYear);
        console.log('sending request to '+getOption.url);
...
function getSeasonList(html){
    console.log('Extracting season list');
    ...
    console.log(seasonObjectList.length+ ' seasons found');
